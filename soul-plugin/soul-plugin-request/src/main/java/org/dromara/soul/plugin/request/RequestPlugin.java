...
import org.dromara.soul.plugin.base.utils.CacheKeyUtils;
import org.dromara.soul.plugin.request.handler.RequestPluginHandler;
...
public class RequestPlugin extends AbstractSoulPlugin {
    ...
    @Override
    protected Mono<Void> doExecute(final ServerWebExchange exchange, final SoulPluginChain chain, final SelectorData selector,
                                   final RuleData rule) {
    ...
    private void setHeaders(final HttpHeaders headers, final ServerHttpRequest request, final RequestHandle requestHandle) {
        ...
        RequestHandle.SoulRequestHeader shenyuReqHeader = requestHandle.getHeader();
    ...
    private MultiValueMap<String, HttpCookie> getCookies(final ServerHttpRequest request, final RequestHandle requestHandle) {
        RequestHandle.SoulCookie shenyuCookie = requestHandle.getCookie();
    ...
    private MultiValueMap<String, String> getQueryParams(final ServerHttpRequest request, final RequestHandle requestHandle) {
        RequestHandle.SoulRequestParameter shenyuReqParameter = requestHandle.getParameter();
